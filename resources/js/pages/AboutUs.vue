        ...
        <div class="py-24" style="background:linear-gradient(131deg, rgb(216 226 255) 26.82%, rgb(232, 229, 254) 96.89%);">
            <div class="mx-10 mt-10 lg:mx-20 lg:mt-0 gap-4 xl:w-7xl xl:mx-auto">
                <div class="flex justify-center">
                    <span class="text-4xl sm:text-6xl xl:text-6xl font-semibold mb-5 sm:mb-8 py-4 text-center max-w-sm md:max-w-lg lg:max-w-2xl xl:max-w-4xl mx-auto font-heading border-b-4 border-fun-blue-500">About Us</span>
                </div>
            </div>
        </div>
                ...
                <div class="mx-10 mt-10 lg:mx-20 lg:mt-0 gap-4 xl:w-7xl xl:mx-auto">
                    <div class="bg-white rounded-3xl p-10">
                        <div v-for="(about, index) in aboutUs" :key="index">
                            <div class="text-4xl font-bold">{{ about.block }}</div>
...
onMounted(async () => {
    document.title = 'About Us - Jindal Thread';
    const res = await apiService.getAboutUs()
    let data = res.data;
    if (data.status) {
